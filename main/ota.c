...
void ota_task(void *data)
{
    ...
    if (lvgl_port_lock(lvgl_lock_timeout)) {
        lv_label_set_text_static(lbl_ln3, "Aktualizacja zakończona");
        lvgl_port_unlock();
    }
    restart_delayed();
err:
    ...
    if (lvgl_port_lock(lvgl_lock_timeout)) {
        lv_label_set_text_static(lbl_ln3, "Aktualizacja nie powiodła się");
        lvgl_port_unlock();
    }
    restart_delayed();
    vTaskDelete(NULL);
...
void ota_start(char *url)
{
    ...
    if (lvgl_port_lock(lvgl_lock_timeout)) {
        ...
        lv_label_set_text_static(lbl_ln3, "Rozpoczynam aktualizację");
